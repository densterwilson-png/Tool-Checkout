...
public class DatabaseSeeder {
    ...
    private static void seedCertifications(Connection conn) throws SQLException {
        ...
        try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            addCertification(pstmt, 1, "FORKLIFT");
            addCertification(pstmt, 1, "WELDING");
            addCertification(pstmt, 2, "FORKLIFT");
            addCertification(pstmt, 3, "WELDING");
...
try (PreparedStatement tx = conn.prepareStatement(txSql)) {

tx.setInt(1, 1);  // Cordless Drill
tx.setInt(2, 1);  // jsmith
tx.setString(3, "CHECKOUT");
tx.setString(4, "2026-06-01");
tx.addBatch();

tx.setInt(1, 6);  // Forklift Key
tx.setInt(2, 6);  // sroberts
tx.setString(3, "CHECKOUT");
tx.setString(4, "2026-06-02");
tx.addBatch();

tx.setInt(1, 7);  // Welding Kit
tx.setInt(2, 7);  // tbrown
tx.setString(3, "CHECKOUT");
tx.setString(4, "2026-06-03");
tx.addBatch();
